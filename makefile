...
VERSION   ?= $(call get_gh_version,Pulse-Eight/libcec)
PLATFORMVERSION ?= master# $(call get_gh_version,Pulse-Eight/platform,PLATFORMVERSION)
PYCECVERSION    ?= $(call get_gh_version,konikvranik/pyCEC,PYCECVERSION)
...
BUILDFLAGS ?= \
	...
	--build-arg PLATFORMVERSION=$(if $(PLATFORMVERSION),$(PLATFORMVERSION),$(error PLATFORMVERSION is not defined)) \
	--build-arg PYCECVERSION=$(if $(PYCECVERSION),$(PYCECVERSION),$(error PYCECVERSION is not defined)) \
...
$(shell \
	...
	then \
		...
		| sed -e 's_v__' -e 's_libcec-__' > $${OFILE}; \
	fi;
	cat $${OFILE};)
endef
# -- }}}
